...
function loadPredictionHistory() {
  try {
    if (fs.existsSync(HISTORY_FILE)) {
      ...
      predictionHistory = {
        hu: Array.isArray(history.hu) ? history.hu : [],
        md5: Array.isArray(history.md5) ? history.md5 : []
      };
      const lastPhien = parsed.lastProcessedPhien || {};
      lastProcessedPhien = {
        hu: lastPhien.hu || null,
        md5: lastPhien.md5 || null
      };
...
function startAutoSaveTask() {
  console.log(`[LC79] Auto-save task started (every ${AUTO_SAVE_INTERVAL/1000}s)`);
...
function startAutoPrediction() {
  console.log(`[LC79] Auto-prediction started (every 3s)`);
...
async function verifyPredictions(type, currentData) {
  ...
  if (learningData[type].predictions.length !== oldLength) {
    updated = true;
    console.log(`[LC79-${type}] Cleaned ${oldLength - learningData[type].predictions.length} old unverified predictions`);
  }
...
function calculateAdvancedPrediction(data, type) {
  ...
  const aiGameId = type === 'hu' ? 'lc79_hu' : 'lc79_md5';
  const aiResult = generateAIPrediction(aiGameId, last50);
...
router.get('/hu/lichsu', async (req, res) => {
  try {
    const data = await fetchDataHu();
    if (data && data.length > 0) {
      await verifyPredictions('hu', data);
...
router.get('/hu/analysis', async (req, res) => {
  try {
    const data = await fetchDataHu();
    if (!data || data.length === 0) {
      return res.status(500).json({ error: 'Không thể lấy dữ liệu' });
...
router.get('/taixiu/analysis', async (req, res) => {
  try {
    const data = await fetchDataMd5();
    if (!data || data.length === 0) {
      return res.status(500).json({ error: 'Không thể lấy dữ liệu' });
...
router.get('/hu/learning', (req, res) => {
...
router.get('/taixiu/learning', (req, res) => {
...
router.get('/hu/ls', async (req, res) => {
  try {
    const data = await fetchDataHu();
    res.json({
      id: 'API BY BO NHAY DZ',
      type: 'LC97 Tài Xỉu Hũ - Lịch sử gốc',
      total: data?.length || 0,
      data: data || []
    });
  } catch (error) {
...
router.get('/taixiu/ls', async (req, res) => {
  try {
    const data = await fetchDataMd5();
    res.json({
      id: 'API BY BO NHAY DZ',
      type: 'LC97 Tài Xỉu - Lịch sử gốc',
      total: data?.length || 0,
      data: data || []
    });
  } catch (error) {
...
router.get('/hu/lsdudoan', (req, res) => {
  ...
  const latestPhien = predictions.length > 0 ? parseInt(predictions[0]?.phien || 0) : 0;
  ...
  res.json({
    id: 'API BY BO NHAY DZ',
    type: 'LC97 Tài Xỉu Hũ - Lịch sử dự đoán',
    total: historyWithStatus.length,
...
router.get('/taixiu/lsdudoan', (req, res) => {
  ...
  res.json({
    id: 'API BY BO NHAY DZ',
    type: 'LC97 Tài Xỉu - Lịch sử dự đoán',
    total: historyWithStatus.length,
    data: historyWithStatus
  });
});
...
router.get('/md5/lsdudoan', (req, res) => {
  ...
  res.json({
    id: 'API BY BO NHAY DZ',
    type: 'LC79 MD5 - Lịch sử dự đoán',
    total: historyWithStatus.length,
    data: historyWithStatus
  });
});

router.get('/reset-learning', (req, res) => {
  learningData = {
    hu: {
      predictions: [],
      patternStats: {},
