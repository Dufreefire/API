...
function calculateAdvancedPrediction(data, type) {
  ...
  let basePrediction = taiCount > xiuCount ? 'Xỉu' : 'Tài';
  let baseConfidence = 50 + Math.abs(taiCount - xiuCount) * 2;

  if (streak >= 4) {
    basePrediction = lastResults[0] === 'Tài' ? 'Xỉu' : 'Tài';
    baseConfidence += 15;
  } else if (streak >= 3) {
    baseConfidence += 5;
  }
  ...
  if (avgSum > 12) {
    if (basePrediction === 'Tài') baseConfidence += 5;
    else baseConfidence -= 5;
  } else if (avgSum < 9) {
    if (basePrediction === 'Xỉu') baseConfidence += 5;
    else baseConfidence -= 5;
  }
...
router.get('/hu/ls', (req, res) => {
  res.json({
    ...
    type: 'Hit Tài Xỉu Hũ - Lịch sử gốc',
    total: historyHu.length,
    data: historyHu
  });
});
...
router.get('/md5/ls', (req, res) => {
  res.json({
    id: 'API BY BO NHAY DZ',
    type: 'Hit Tài Xỉu MD5 - Lịch sử gốc',
    total: historyMd5.length,
    data: historyMd5
  });
});
...
router.get('/hu/lsdudoan', (req, res) => {
  ...
  res.json({
    id: 'API BY BO NHAY DZ',
    type: 'Hit Tài Xỉu Hũ - Lịch sử dự đoán',
...
router.get('/md5/lsdudoan', (req, res) => {
  ...
  res.json({
    id: 'API BY BO NHAY DZ',
    type: 'Hit Tài Xỉu MD5 - Lịch sử dự đoán',
    total: historyWithStatus.length,
    data: historyWithStatus
  });
});
