...
function startAutoSaveTask() {
  console.log(`[B52] Auto-save task started (every ${AUTO_SAVE_INTERVAL/1000}s)`);
...
function startAutoPrediction() {
  console.log(`[B52] Auto-prediction started (every 3s)`);
...
router.get('/taixiu/lsdudoan', (req, res) => {
  ...
  const latestPhien = predictionHistory.b52?.length > 0 && predictionHistory.b52[0]?.phien 
    ? parseInt(predictionHistory.b52[0].phien) 
    : (predictions.length > 0 ? parseInt(predictions[0]?.phien || 0) : 0);
  ...
  res.json({
    id: 'API BY BO NHAY DZ',
    type: 'B52 Tài Xỉu - Lịch sử dự đoán',
    total: historyWithStatus.length,
