...
const EXTERNAL_API_URL = 'http://luck8bot.com/api/GetNewLottery/LT_TaixiuMD5';
  ...
  try {
    ...
    if (json && json.state === 1 && json.data) {
      ...
      if (phien !== lastPhien) {
        ...
        console.log(`[Luck8-MD5] 🎲 Phiên ${phien}: ${diceArr.join('-')} = ${sum} (${ketqua})`);
      }
    }
  } catch (error) {
    console.error('[Luck8-MD5] Error polling data:', error.message);
  }
}
...
function startPolling() {
  ...
  console.log('[Luck8-MD5] Starting external API polling...');
...
router.get('/taixiu/ls', (req, res) => {
    res.json({
      id: 'API BY BO NHAY DZ',
      type: 'Luck8 MD5 Tài Xỉu - Lịch sử gốc',
...
router.get('/taixiu/lsdudoan', (req, res) => {
  ...
  const historyWithStatus = predictionHistory.map(p => {
    ...
    res.json({
    id: 'API BY BO NHAY DZ',
    type: 'Luck8 MD5 Tài Xỉu - Lịch sử dự đoán',
...
function verifyPredictions() {
  for (const pred of predictionHistory) {
    ...
    if (actual) {
      ...
      pred.actual = actual.Ket_qua;
      pred.isCorrect = pred.du_doan === actual.Ket_qua;
        console.log(`[Luck8-MD5] Verified phien ${pred.phien}: ${pred.isCorrect ? '✅' : '❌'}`);
    }
  }
}
  ...
  try {
    ...
    if (lastPredictedPhien !== nextPhien) {
      ...
      if (!existingPred) {
        ...
        console.log(`[Luck8-MD5-Auto] Phien ${nextPhien}: ${prediction.du_doan} (${prediction.do_tin_cay}%)`);
      }
    }
  } catch (error) {
    console.error('[Luck8-MD5-Auto] Error:', error.message);
  }
}
